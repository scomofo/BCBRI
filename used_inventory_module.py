...
try:
    ...
    from modules.sharepoint_manager import SharePointExcelManager
except ImportError:
...
class UsedInventoryModule(QWidget):
    ...
    def _show_status_message(self, message, timeout=3000):
        ...
        if self.main_window and hasattr(self.main_window, 'statusBar'):
            # --- FIX: Access statusBar as an attribute, don't call it ---
            status_bar_obj = self.main_window.statusBar
            # Check if the object exists and has the showMessage method
            if status_bar_obj and hasattr(status_bar_obj, 'showMessage'):
                status_bar_obj.showMessage(message, timeout)
            else:
                # Fallback print if status bar object is invalid somehow
                print(f"Status (UsedInv): {message} (statusBar object invalid or lacks showMessage)")
        else:
            # Fallback print if no main window or status bar attribute
            print(f"Status (UsedInv): {message} (main_window or statusBar not available)")
    ...
    def load_inventory_data(self):
        ...
        self.table.setRowCount(0) # Reset row count
        ...
        if sheet_data is None:
            ...
            QMessageBox.warning(self, "Load Error", f"Failed to load data from sheet '{sheet_name}'. Check logs.")
            return
        ...
        if not sheet_data or len(sheet_data) < 1: # Check if sheet_data is empty or just header
            ...
            QMessageBox.information(self, "Info", f"Sheet '{sheet_name}' appears to be empty.")
            return
        ...
        try:
             ...
             desc_col_index = self.inventory_headers.index(desc_col_name)
             self.table.horizontalHeader().setSectionResizeMode(desc_col_index, QHeaderView.Stretch)
        except ValueError:
             self.table.horizontalHeader().setStretchLastSection(True) # Fallback: stretch last column
    ...
    def closeEvent(self, event):
        """Placeholder for cleanup if needed when module is closed/hidden."""
        print("DEBUG: UsedInventoryModule closeEvent triggered (Placeholder).")
        event.accept()
...
if __name__ == '__main__':
    ...
    class MockMainWindow:
        def __init__(self):
            self.statusBar = self # Use self for mock status bar

        # Mock the statusBar method itself
        def statusBar(self):
             return self
    ...
    # Passing data_path is not strictly needed here unless UsedInventory needs it internally
    inventory_module = UsedInventoryModule(main_window=mock_main, sharepoint_manager=mock_sp)
