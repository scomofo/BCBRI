...
class CalculatorModule(QWidget):
    ...
    def calculate(self):
        ...
        # Calculate CAD cost from USD cost and exchange rate
        if usd is not None and rate is not None and sender_name != "cad":
            cad = usd * rate
            self.cad_cost.setText(format_number(cad))
        # Calculate USD cost from CAD cost and exchange rate
        elif cad is not None and rate is not None and sender_name != "usd" and rate != 0:
            usd = cad / rate
            self.usd_cost.setText(format_number(usd))
        # Calculate exchange rate from USD and CAD costs
        elif usd is not None and cad is not None and sender_name != "rate" and usd != 0:
            rate = cad / usd
            self.exchange_rate.setText(format_number(rate))

        # Calculate margin from markup (markup to margin)
        if markup is not None and sender_name == "markup":
            margin = (markup / (100 + markup)) * 100
            self.margin.setText(format_number(margin))
        
        # Calculate markup from margin (margin to markup)
        elif margin is not None and sender_name == "margin":
            markup = (margin / (100 - margin)) * 100
            self.markup.setText(format_number(markup))
        
        # Calculate revenue from CAD cost and markup
        if cad is not None and markup is not None and sender_name != "revenue":
            revenue = cad * (1 + markup/100)
            self.revenue.setText(format_number(revenue))
        
        # Calculate markup from CAD cost and revenue
        elif cad is not None and revenue is not None and sender_name == "revenue" and cad != 0:
            markup = ((revenue / cad) - 1) * 100
            self.markup.setText(format_number(markup))
            # Also update margin based on the calculated markup
            margin = (markup / (100 + markup)) * 100
            self.margin.setText(format_number(margin))
        
        # Calculate CAD cost from revenue and markup
        elif revenue is not None and markup is not None and sender_name != "cad" and markup != -100:
            cad = revenue / (1 + markup/100)
            self.cad_cost.setText(format_number(cad))
            # If we have the exchange rate, also update USD cost
